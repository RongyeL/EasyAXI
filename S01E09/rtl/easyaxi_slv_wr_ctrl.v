...
// Last Modified : 2026-03-02 21:51
// ---------------------------------------------------------------------------------
...
// Slave Write Internal Interface
    output wire                      s_wr_data_en,
    output wire  [`AXI_ID_W    -1:0] s_wr_data_id,
    output wire  [`AXI_ADDR_W  -1:0] s_wr_data_addr,
    output wire  [127:0]             s_wr_data_data,   // 128-bit data for internal interface
    output wire  [15:0]              s_wr_data_strb    // 16-byte strobe
);
...
localparam MAX_GET_RESP_DLY = `AXI_RESP_GET_CNT_W'h2;      // Reduced delay for faster simulation
...
reg  [MAX_BURST_LEN*`AXI_DATA_W -1:0] wr_data_strb_r [OST_DEPTH-1:0];
...
wire                     wr_data_last;        
reg                      wr_data_last_r;
reg  [OST_CNT_W    -1:0] wr_data_ptr_r;
...
//--------------------------------------------------------------------------------
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            wr_data_strb_r[i] <= #DLY {((`AXI_DATA_W/8)*MAX_BURST_LEN){1'b0}};
        end
        else if (wr_buff_set && (wr_set_ptr == i)) begin
            wr_data_vld_r [i] <= #DLY {MAX_BURST_LEN{1'b0}};
            wr_data_buff_r[i] <= #DLY {(`AXI_DATA_W*MAX_BURST_LEN){1'b0}};
            ...
            wr_data_buff_r[i][((wr_curr_index_r[i]-1)*`AXI_DATA_W) +: `AXI_DATA_W] <= #DLY axi_slv_wdata;
            wr_data_strb_r[i][((wr_curr_index_r[i]-1)*(`AXI_DATA_W/8)) +: (`AXI_DATA_W/8)] <= #DLY axi_slv_wstrb;
        end
    end
    
    assign wr_resp_get   [i] = wr_data_last_r && (wr_data_ptr_r == i);
    assign wr_resp_err   [i] = `AXI_RESP_OKAY;
end
endgenerate
//--------------------------------------------------------------------------------
// WLAST -> internal write trigger
// Slave does not write memory per beat; it writes the entire burst in one cycle.
//   Trigger: wvalid & wready & wlast -> wr_data_last_r pulse
//   Effect: s_wr_data_en asserts for 1 cycle, outputting collected 128-bit data
//--------------------------------------------------------------------------------
// resp wait process
//--------------------------------------------------------------------------------
always @(posedge clk or negedge rst_n) begin
    if (~rst_n) begin
        wr_data_last_r <= #DLY 1'b0;
        wr_data_ptr_r <= #DLY {OST_CNT_W{1'b0}};
    end
    else if (wr_data_en && wr_data_last) begin
        // Memory response received, start sending beats
        wr_data_last_r <= #DLY 1'b1;
        wr_data_ptr_r <= #DLY wr_data_ptr;
    end
    else begin
        wr_data_last_r <= #DLY 1'b0;
    end
...
//--------------------------------------------------------------------------------
// Output write data through internal interface when complete burst is received
reg                      s_wr_data_en_r;
reg  [`AXI_ID_W    -1:0] s_wr_data_id_r;
reg  [`AXI_ADDR_W  -1:0] s_wr_data_addr_r;
reg  [127:0]             s_wr_data_data_r;  // 128-bit data for internal interface
reg  [15:0]              s_wr_data_strb_r;  // 16-byte strobe

always @(posedge clk or negedge rst_n) begin
    if (~rst_n) begin
        s_wr_data_en_r   <= #DLY 1'b0;
        s_wr_data_id_r   <= #DLY {`AXI_ID_W{1'b0}};
        s_wr_data_addr_r <= #DLY {`AXI_ADDR_W{1'b0}};
        s_wr_data_data_r <= #DLY 128'h0;
        s_wr_data_strb_r <= #DLY 16'h0;
    end
    else if (wr_data_last_r) begin
        // Send complete 128-bit data to memory when last beat is received
        s_wr_data_en_r   <= #DLY 1'b1;
        s_wr_data_id_r   <= #DLY wr_id_buff_r[wr_data_ptr_r];
        s_wr_data_addr_r <= #DLY wr_addr_buff_r[wr_data_ptr_r];  // Use start address
        // Truncate to 128-bit (16B) for internal interface
        s_wr_data_data_r <= #DLY wr_data_buff_r[wr_data_ptr_r][127:0];
        s_wr_data_strb_r <= #DLY wr_data_strb_r[wr_data_ptr_r][15:0];
    end
    else begin
        s_wr_data_en_r   <= #DLY 1'b0;
    end
...
assign s_wr_data_en   = s_wr_data_en_r;
assign s_wr_data_id   = s_wr_data_id_r;
assign s_wr_data_addr = s_wr_data_addr_r;
assign s_wr_data_data = s_wr_data_data_r;
assign s_wr_data_strb = s_wr_data_strb_r;
